...
impl Annotations {
    ...
    pub(crate) fn add(&mut self, xot: &Xot, doc: Node) {
    ...
    pub(crate) fn get(&self, node: Node) -> Option<&Annotation> {
        self.map.get(&node)
    }

    pub(crate) fn document_order(&self, node: Node) -> DocumentOrder {
        self.get(node)
...
#[generator(yield(Node))]
fn document_descendants(xot: &Xot, doc: Node) {
    match doc {
        Node::Xot(node) => {
            if !xot.is_root(node) {
            ...
            for descendant in xot.descendants(node) {
                yield_!(Node::Xot(descendant));
                if let Some(element) = xot.element(descendant) {
                    for prefix in element.prefixes().keys() {
                        yield_!(Node::Namespace(descendant, *prefix));
                    }
                    for attr_name in element.attributes().keys() {
                        yield_!(Node::Attribute(descendant, *attr_name));
                    }
                }
            }
        }
        Node::Attribute(..) | Node::Namespace(..) => {
            panic!("node is not a document");
        }
    }
}
...
mod tests {
    ...
    fn test_single_document() {
        ...
        annotations.add(&xot, Node::Xot(doc));
    ...
    fn test_multiple_documents() {
        ...
        let root1 = Node::Xot(root1);
        let c = Node::Xot(c);
        let d = Node::Xot(d);

        let mut annotations = Annotations::new();
        annotations.add(&xot, Node::Xot(doc0));
        annotations.add(&xot, Node::Xot(doc1));
    ...
    fn test_attributes() {
        ...
        let a = Node::Attribute(x, xot.name("a").unwrap());
        let b = Node::Attribute(x, xot.name("b").unwrap());
        let c = Node::Attribute(y, xot.name("c").unwrap());
        let x = Node::Xot(x);
        let y = Node::Xot(y);
