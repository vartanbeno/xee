    ...
    P: Fn(Node, &State, &Context) -> Result<V>,
{
    fn parse(
        ...
        loop {
            let (item, next) = optional_parser.parse(current_node, state, context)?;
    ...
    for CombinedParser<TA, TB, PA, PB>
{
    fn parse(
        ...
        let (a, node) = self.first.parse(node, state, context)?;
        let (b, node) = self.second.parse(node, state, context)?;
        Ok(((a, b), node))
    }
}
    ...
    for IgnoreRightCombinedParser<TA, TB, PA, PB>
{
    fn parse(
        ...
        let (a, node) = self.first.parse(node, state, context)?;
        let (_b, node) = self.second.parse(node, state, context)?;
        Ok((a, node))
    }
}
...
mod tests {
    ...
    fn parse(s: &str) -> (State, Context, Option<Node>) {
        let mut xot = Xot::new();
    ...
    fn test_optional_present() {
        let (state, context, next) = parse("<outer><a /></outer>");
    ...
    fn test_optional_present_but_parse_error() {
        ...
        let r: Result<(Option<Value>, Option<Node>)> =
            optional_parser.parse(next, &state, &context);
    ...
    fn test_optional_unexpected_node() {
        let (state, context, node) = parse("<outer><a /></outer>");
        ...
        let (item, next): (Option<Value>, Option<Node>) =
            optional_parser.parse(node, &state, &context).unwrap();
        assert_eq!(item, None);
        assert_eq!(next, node);
    }
    ...
    fn test_optional_not_present() {
        let (state, context, next) = parse("<outer></outer>");
    ...
    fn test_end_found() {
        ...
        let r = end_parser.parse(next, &state, &context);
    ...
    fn test_two_optional_both_present() {
        let (mut state, context, next) = parse("<outer><a /><b /></outer>");
    ...
    fn test_two_optional_only_a_present() {
        let (mut state, context, next) = parse("<outer><a /></outer>");
        let names = TestNames::new(&mut state.xot);
    ...
    fn test_two_optional_only_b_present() {
        let (mut state, context, next) = parse("<outer><b /></outer>");
        let names = TestNames::new(&mut state.xot);
    ...
    fn test_two_optional_neither_present() {
        let (mut state, context, next) = parse("<outer></outer>");
    ...
    fn test_two_optional_unexpected() {
        let (mut state, context, next) = parse("<outer><c /></outer>");
        let names = TestNames::new(&mut state.xot);
    ...
    #[test]
    fn test_many() {
        let (state, context, next) = parse("<outer><a /><a /><a /></outer>");
    ...
    fn test_optional_then_many() {
        let (mut state, context, next) = parse("<outer><a /><b /><b /></outer>");
        ...
        let (optional_item, next) = optional_parser.parse(next, &state, &context).unwrap();
        let (many_items, next) = many_parser.parse(next, &state, &context).unwrap();
    ...
    fn test_combine() {
        ...
        let ((optional_item, many_items), next) = combined.parse(next, &state, &context).unwrap();
    ...
    fn test_combine_3_values() {
        ...
        let (((optional_item, many_items), _), next) =
            combined.parse(next, &state, &context).unwrap();
    ...
    fn test_combine_then_ignore() {
        let (mut state, context, next) = parse("<outer><b /><b /></outer>");
        ...
        let (many_items, next) = combined.parse(next, &state, &context).unwrap();
    ...
    #[test]
    fn test_attribute() {
        let (mut state, context, next) = parse(r#"<outer><b foo="FOO"/></outer>"#);
        ...
        let (item, _next) = parser.parse(next, &state, &context).unwrap();
