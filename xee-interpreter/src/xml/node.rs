...
// fn node_name_id(xot: &Xot, node: xot::Node) -> Option<xot::NameId> {
...
//         xot::Value::Root => None,
//         xot::Value::Attribute(attribute) => Some(attribute.name()),
//         xot::Value::Namespace(_) => None,
//     }
// }

// fn node_name(xot: &Xot, node: xot::Node) -> Option<Name> {
//     let name_id = node_name_id(xot, node)?;
//     Some(Name::from_xot(name_id, xot))
// }

// fn node_local_name(xot: &Xot, node: xot::Node) -> String {
//     if let Some(name) = node_name_id(xot, node) {
...
// fn node_namespace_uri(xot: &Xot, node: xot::Node) -> String {
//     if let Some(name) = node_name_id(xot, node) {
...
// fn node_typed_value(xot: &Xot, node: xot::Xot) -> Vec<atomic::Atomic> {
//     // for now we don't know any types of nodes yet
//     let s = node_string_value(xot, node);
//     vec![atomic::Atomic::Untyped(Rc::new(s))]
// }

// fn node_type_annotation(xot: &Xot, node: xot::Xot) -> Xs {
//     // for now we don't know any types of nodes yet
//     Xs::UntypedAtomic
// }

// fn string_value(xot: &Xot, node: xot::Node) -> String {
//     match xot.value(*node) {
...
//         xot::Value::Attribute(attribute) => attribute.value().to_string(),
//         xot::Value::Namespace(namespace) => {
//             let namespace_id = namespace.namespace();
//             xot.namespace_str(namespace_id).unwrap()
//         }
//     }
// }
